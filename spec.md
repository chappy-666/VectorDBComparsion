...
- [x] `docker-compose.yml` の作成（完了）
- [x] `devcontainer.json` の作成（完了）
- [x] 各サービスの起動確認（完了）
- Ollamaモデルは未pull（初回に `llama3.2` を pull する必要あり）

## 開発環境の構成

- devcontainerでbackendコンテナにアタッチ
- ワークスペース: `/workspace`（プロジェクトルート）
- backendコード: `/app`
- 各サービスへの接続:
  - Ollama: `http://ollama:11434`
  - Qdrant: `http://qdrant:6333`
  - pgvector: `pgvector:5432`（user: postgres, password: postgres, db: vectordb）

## 残タスク

- [ ] Ollamaにモデルをpull（llama3.2）
- [ ] バックエンドからQdrant/pgvectorへの接続実装
- [ ] Embeddingパイプライン構築（ドキュメント → ベクトル化 → 両DBに格納）
- [ ] LangGraphによるRAGエージェント構築
- [ ] DB切り替え・並列検索APIの実装
- [ ] フロントエンド実装（検索UI、DB切り替え、比較表示）
- [ ] 比較結果（レイテンシ・精度）の可視化